...
def get(url):
    try:
        r = subprocess.run(["curl", "-k", "--ssl-no-revoke", "--max-time", "25", "-A", UA, "-s", url],
...
def sql_upsert(kind, accepted):
    ...
    for r in accepted.values():
        vals.append(f"({q(kind)},{q(r['issue'])},{q(r['draw_date'])},{q(json.dumps(r['main']))}::json,"
    ...
    if not vals:
        return None
    return ("INSERT INTO collect_shadow (kind,issue,draw_date,main_numbers,special_numbers,sales,pool_amount,prizes,source,collected_at) VALUES "
            + ",".join(vals) + " ON CONFLICT (kind,issue) DO NOTHING;")
...
def main():
    ...
    for kind in SOURCES:
        ...
        if not feeds:
            unreachable.append(kind); print(f"[{kind}] 全部源不可达：{dead}"); continue
        if dead:
    ...
    with psycopg.connect(os.environ["DATABASE_URL"]) as conn:
        with conn.cursor() as cur:
            cur.execute(DDL); cur.execute(DDL_LOG)
            for st in built: cur.execute(st)
            cur.execute(LOG_SQL)
            cur.execute("SELECT kind,mismatch,collector_ahead,verdict FROM collect_shadow_log ORDER BY checked_at DESC LIMIT 8;")
            rows = cur.fetchall()
            conn.commit()
    print("最新账本：", rows)
