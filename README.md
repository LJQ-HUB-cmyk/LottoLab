...
| 大乐透 DLT | 前 5/35 + 后 2/12 | 完整（ROI 除外，奖金规则有历史版本差异） |
...
每次推送与 PR 自动运行三条流水：后端（PostgreSQL 17 真库回归）、浏览器（Playwright 本地与云端两种模式）、容器（构建→灌数据→重建→校验数据与快照保留）。后端门禁为 `pip check`、Ruff 检查与格式、`mypy`、157 项 pytest；前端门禁为 Prettier 与构建。文档改动需核对命令、链接与真实渲染。
